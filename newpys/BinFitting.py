        ...
        if t_val == 0:
            ...
            try:
                ...
                for x in range(len(new_bins)):
                    ...
                    background = background.resize((new_bins[x][0] * 10, new_bins[x][1] * 10))
                    ...
                    for idnt, h, w in recs_details:
                        try:
                            ...
                            temp = blue.resize((rect.width * 10, rect.height * 10))
                            draw = ImageDraw.Draw(temp)
                            msg = str(idnt) + "\n" + str(h) + 'x' + str(w)
                            w, h = draw.textsize(msg)
                            draw.text((((rect.width * 10 - w)) / 2, ((rect.height * 10 - h)) / 2), msg, fill="black")
                            background.paste(im=temp, box=(rect.x * 10, rect.y * 10))
                            count += 1
                        except:
                            break
                    try:
                        for a in range(len(fit_packer[x])):
                            recs_details.pop(0)
                    except:
                        continue

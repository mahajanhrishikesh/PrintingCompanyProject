...
def Update_list(paper_type,gsm):
    try:
        connection = mysql.connector.connect(host="localhost",user="root",passwd='',database="traning_database")
        cursor = connection.cursor()
        query = "SELECT order_id,length,width,num_of_copies FROM printing_request WHERE master_printer_job = 1 AND jobcard_type IS (%s) AND gsm = (%s)"
        val =[paper_type+'%',gsm]
...
def Update_bin(paper_type,gsm):
    try:
        ...
        connection = mysql.connector.connect(host="localhost",user="root",passwd='',database="traning_database")
        cursor = connection.cursor()
        query = "SELECT length,width,num_of_cards FROM job_card WHERE Name IS (%s) AND gsm = (%s)"
        val =[paper_type+'%',gsm]
